...
def file_based_input_fn_builder(input_file, num_cands, seq_length, is_training,
  ...
  name_to_features = {
      "input_ids": tf.FixedLenFeature([2*num_cands, seq_length], tf.int64),
      "input_mask": tf.FixedLenFeature([2*num_cands, seq_length], tf.int64),
      "segment_ids": tf.FixedLenFeature([2*num_cands, seq_length], tf.int64),
      "mention_id": tf.FixedLenFeature([2*num_cands, seq_length], tf.int64),
      "labels": tf.FixedLenFeature([2*num_cands], tf.int64),
      "uid_a": tf.FixedLenFeature([2*num_cands, NUM_UID_BYTES], tf.int64),
      "uid_b": tf.FixedLenFeature([2*num_cands, NUM_UID_BYTES], tf.int64),
  }
...
def create_zeshel_model(bert_config, num_cands, is_training, input_ids, input_mask,
  ...
  tf.Print(hidden_size, [hidden_size], '####################: ')
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
                     ...
                     num_cands, use_one_hot_embeddings):
  """Returns `model_fn` closure for TPUEstimator."""

  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    """The `model_fn` for TPUEstimator."""

    ## NOTE: ^^^ this `labels` is unused and most likely empty
    ...
    uid_a = features["uid_a"]
    uid_b = features["uid_b"]
    ...
    (total_loss, per_example_loss, logits, probabilities) = create_zeshel_model(
        bert_config, num_cands, is_training, input_ids, input_mask,
        segment_ids, mention_ids, labels, use_one_hot_embeddings)
...
def main(_):
  ...
  if FLAGS.do_predict:
    ...
    mention_reps = defaultdict(list)
    for prediction in result:
      for i in range(len(prediction['probabilities'])):
        uid_a = struct.pack("=LLLL", *list(prediction["uid_a"][i])).decode("utf-8")
        uid_b = struct.pack("=LLLL", *list(prediction["uid_b"][i])).decode("utf-8")
        mention_reps[uid_a].append((uid_b, prediction['probabilities'][i]))
