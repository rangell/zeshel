...
def create_coref_candidate_sets(mention, all_mentions, tfidf_candidates,
                                entity2mention, entity2mention_w_cand):
  ...
  if len(pos_cand) == 0:
    pos_cand = [m for m in entity2mention[mention_label] if m is not mention]

  if len(pos_cand) < num_cands:
    pos_cand.extend([m for m in entity2mention_w_cand[mention_label] if m is not mention])
